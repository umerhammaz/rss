 ...
 * @subpackage sxmldom_simple_html_dom
 */
...
function sxmldom_file_get_html($url, $use_include_path = false, $context=null, $offset = -1, $maxLen=-1, $lowercase = true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
{
    // We DO force the tags to be terminated.
    $dom = new sxmldom_simple_html_dom(null, $lowercase, $forceTagsClosed, $target_charset, $stripRN, $defaultBRText, $defaultSpanText);
...
function sxmldom_str_get_html($str, $lowercase=true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
{
    $dom = new sxmldom_simple_html_dom(null, $lowercase, $forceTagsClosed, $target_charset, $stripRN, $defaultBRText, $defaultSpanText);
...
// dump html dom tree
function sxmldom_dump_html_tree($node, $show_attr=true, $deep=0)
{
    $node->dump($node);
}
 ...
 * @package PlaceLocalInclude
 */
class sxmldom_simple_html_dom_node
{
...
class sxmldom_simple_html_dom
{
    ...
    protected function prepare($str, $lowercase=true, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
    {
        ...
        $this->root = new sxmldom_simple_html_dom_node($this);
        $this->root->tag = 'root';
    ...
    protected function parse()
    {
        ...
        // text
        $node = new sxmldom_simple_html_dom_node($this);
        ++$this->cursor;
    ...
    protected function read_tag()
    {
        ...
        {
            $this->char = (++$this->pos<$this->size) ? $this->doc[$this->pos] : null; // next
            // This represents the change in the sxmldom_simple_html_dom trunk from revision 180 to 181.
    ...
    function createElement($name, $value=null) {return @sxmldom_str_get_html("<$name>$value</$name>")->first_child();}
    function createTextNode($value) {return @end(sxmldom_str_get_html($value)->nodes);}
    function getElementById($id) {return $this->find("#$id", 0);}
    function getElementsById($id, $idx=null) {return $this->find("#$id", $idx);}
    function getElementByTagName($name) {return $this->find($name, 0);}
