...
* ------------------------------------------------------
* Original URL: http://lab.arc90.com/experiments/wp_automatic_Readability/js/wp_automatic_Readability.js
* Arc90's project URL: http://lab.arc90.com/experiments/wp_automatic_Readability/
* JS Source: http://code.google.com/p/arc90labs-wp_automatic_Readability
...
* ------------------------------------------------------
* Arc90's wp_automatic_Readability is designed to run in the browser. It works on the DOM 
...
* match Arc90's wp_automatic_Readability. (For example, if a web page contains CSS style 
* rules or Javascript code which hide certain HTML elements from display, 
* Arc90's wp_automatic_Readability will dismiss those from consideration but our PHP port, 
* unable to understand CSS or Javascript, will not know any better.)
* 
* Another significant difference is that the aim of Arc90's wp_automatic_Readability is 
...
class wp_automatic_Readability
{
	...
	**/
	...
	/**
	* Create instance of wp_automatic_Readability
	...
	*/
	...
	/**
	* Runs wp_automatic_Readability.
	* 
	* Workflow:
	*  1. Prep the document by removing script tags, css, etc.
	*  2. Build wp_automatic_Readability's DOM tree.
	...
	**/
	public function init()
	{
		...
		if (!$articleContent) {
			...
			$articleContent->setAttribute('id', 'wp_automatic_Readability-content');
			$articleContent->innerHTML = '<p>Sorry, wp_automatic_Readability was unable to parse this page for content.</p>';		
		}
	...
	*/
	...
	/**
	* Prepare the HTML document for wp_automatic_Readability to scrape it.
	...
	**/
	protected function prepDocument() {
		...
		*/
		...
		$this->body->setAttribute('id', 'wp_automatic_ReadabilityBody');
		...
		//document.body.innerHTML = document.body.innerHTML.replace(wp_automatic_Readability.regexps.replaceBrs, '</p><p>').replace(wp_automatic_Readability.regexps.replaceFonts, '<$1span>');
	...
	**/
	public function addFootnotes($articleContent) {
		...
		$footnotesWrapper->setAttribute('id', 'wp_automatic_Readability-footnotes');
		$footnotesWrapper->innerHTML = '<h3>References</h3>';
		
		$articleFootnotes = $this->dom->createElement('ol');
		$articleFootnotes->setAttribute('id', 'wp_automatic_Readability-footnotes-list');
		...
		for ($i = 0; $i < $articleLinks->length; $i++)
		{
			...
			if ((strpos($articleLink->getAttribute('class'), 'wp_automatic_Readability-DoNotFootnote') !== false) || preg_match($this->regexps['skipFootnoteLink'], $linkText)) {
				continue;
			}
			...
			$refLink->setAttribute('href', '#wp_automatic_ReadabilityFootnoteLink-' . $linkCount);
			$refLink->innerHTML = '<small><sup>[' . $linkCount . ']</sup></small>';
			$refLink->setAttribute('class', 'wp_automatic_Readability-DoNotFootnote');
			...
			$articleLink->setAttribute('name', 'wp_automatic_ReadabilityLink-' . $linkCount);

			$footnote->innerHTML = '<small><sup><a href="#wp_automatic_ReadabilityLink-' . $linkCount . '" title="Jump to Link in Article">^</a></sup></small> ';

			$footnoteLink->innerHTML = ($footnoteLink->getAttribute('title') != '' ? $footnoteLink->getAttribute('title') : $linkText);
			$footnoteLink->setAttribute('name', 'wp_automatic_ReadabilityFootnoteLink-' . $linkCount);
	...
	/**
	* Reverts P elements with class 'wp_automatic_Readability-styled'
	* to text nodes - which is what they were before.
	*
	* @param DOMElement
	* @return void
	*/
	function revertwp_automatic_ReadabilityStyledElements($articleContent) {
		$xpath = new DOMXPath($articleContent->ownerDocument);
		$elems = $xpath->query('.//p[@class="wp_automatic_Readability-styled"]', $articleContent);
	...
	*/
	function prepArticle($articleContent) {
		...
		if ($this->revertForcedParagraphElements) {
			$this->revertwp_automatic_ReadabilityStyledElements($articleContent);
		}
	...
	/**
	* Initialize a node with the wp_automatic_Readability object. Also checks the
	...
	**/
	protected function initializeNode($node) {
		$wp_automatic_Readability = $this->dom->createAttribute('wp_automatic_Readability');
		$wp_automatic_Readability->value = 0; // this is our contentScore
		$node->setAttributeNode($wp_automatic_Readability);		         

		switch (strtoupper($node->tagName)) { // unsure if strtoupper is needed, but using it just in case
			case 'DIV':
				$wp_automatic_Readability->value += 5;
				break;

			case 'PRE':
			case 'TD':
			case 'BLOCKQUOTE':
				$wp_automatic_Readability->value += 3;
				break;
		...
		$wp_automatic_Readability->value += $this->getClassWeight($node);
	}
	...
	protected function grabArticle($page=null) {
				...
				else
				{
					...
					for ($i = 0, $il = $node->childNodes->length; $i < $il; $i++) {
						...
						if ($childNode->nodeType == 3) { // XML_TEXT_NODE
							...
							$p->setAttribute('class', 'wp_automatic_Readability-styled');
		...
		**/
		...
		for ($pt=0; $pt < count($nodesToScore); $pt++) {
			...
			/* Initialize wp_automatic_Readability data for the parent. */
			if (!$parentNode->hasAttribute('wp_automatic_Readability')) 
			{
				$this->initializeNode($parentNode);
				$candidates[] = $parentNode;
			}

			/* Initialize wp_automatic_Readability data for the grandparent. */
			if ($grandParentNode && !$grandParentNode->hasAttribute('wp_automatic_Readability') && isset($grandParentNode->tagName))
			{
			...
			$parentNode->getAttributeNode('wp_automatic_Readability')->value += $contentScore;

			if ($grandParentNode) {
				$grandParentNode->getAttributeNode('wp_automatic_Readability')->value += $contentScore/2;             
			}
		}
		...
		**/
		...
		for ($c=0, $cl=count($candidates); $c < $cl; $c++)
		{
			...
			**/
			$wp_automatic_Readability = $candidates[$c]->getAttributeNode('wp_automatic_Readability');
			$wp_automatic_Readability->value = $wp_automatic_Readability->value * (1-$this->getLinkDensity($candidates[$c]));

			$this->dbg('Candidate: ' . $candidates[$c]->tagName . ' (' . $candidates[$c]->getAttribute('class') . ':' . $candidates[$c]->getAttribute('id') . ') with score ' . $wp_automatic_Readability->value);

			if (!$topCandidate || $wp_automatic_Readability->value > (int)$topCandidate->getAttribute('wp_automatic_Readability')) {
		...
		**/
		$articleContent        = $this->dom->createElement('div');
		@$articleContent->setAttribute('id', 'wp_automatic_Readability-content');
		$siblingScoreThreshold = max(10, ((int)$topCandidate->getAttribute('wp_automatic_Readability')) * 0.2);
		...
		for ($s=0, $sl=$siblingNodes->length; $s < $sl; $s++)
		{
			...
			$this->dbg('Looking at sibling node: ' . $siblingNode->nodeName . (($siblingNode->nodeType === XML_ELEMENT_NODE && $siblingNode->hasAttribute('wp_automatic_Readability')) ? (' with score ' . $siblingNode->getAttribute('wp_automatic_Readability')) : ''));

			//dbg('Sibling has score ' . ($siblingNode->wp_automatic_Readability ? siblingNode.wp_automatic_Readability.contentScore : 'Unknown'));
			...
			if ($siblingNode->nodeType === XML_ELEMENT_NODE && $siblingNode->getAttribute('class') == $topCandidate->getAttribute('class') && $topCandidate->getAttribute('class') != '') {
				$contentBonus += ((int)$topCandidate->getAttribute('wp_automatic_Readability')) * 0.2;
			}

			if ($siblingNode->nodeType === XML_ELEMENT_NODE && $siblingNode->hasAttribute('wp_automatic_Readability') && (((int)$siblingNode->getAttribute('wp_automatic_Readability')) + $contentBonus) >= $siblingScoreThreshold)
			{
				$append = true;
			}
			...
			if ($append)
			{
				...
				/* To ensure a node does not interfere with wp_automatic_Readability styles, remove its classnames */
	...
	public function cleanConditionally($e, $tag) {
		...
		*/
		for ($i=$curTagsLength-1; $i >= 0; $i--) {
			...
			$contentScore = ($tagsList->item($i)->hasAttribute('wp_automatic_Readability')) ? (int)$tagsList->item($i)->getAttribute('wp_automatic_Readability') : 0;
			
			$this->dbg('Cleaning Conditionally ' . $tagsList->item($i)->tagName . ' (' . $tagsList->item($i)->getAttribute('class') . ':' . $tagsList->item($i)->getAttribute('id') . ')' . (($tagsList->item($i)->hasAttribute('wp_automatic_Readability')) ? (' with score ' . $tagsList->item($i)->getAttribute('wp_automatic_Readability')) : ''));
