...
class WpAutomaticTelegram extends wp_automatic
{

    /*
     * ---* Fetch a new list of items
     */
    public function telegram_fetch_items($keyword, $camp)
    {
        echo "<br>So I should now get some items from Telegram ...";
        ...
        // items url
        $cg_rd_page = wp_automatic_trim($camp_general['cg_te_page']);
        $cg_rd_page_md = md5($cg_rd_page);

        //if empty page return false and ask the user to add a correct telegram page URL on the format https://t.me/s/id
        if (wp_automatic_trim($cg_rd_page) == '') {
            echo '<br>Telegram page URL is empty please visit telegram.com and get a correct one on the format https://t.me/s/id ';
            return false;
        }

        //replace telegram.me with t.me
        $cg_rd_page = wp_automatic_str_replace('telegram.me', 't.me', $cg_rd_page);

        //if page does not contain http, append https://t.me/s/ to the beginning
        if (!(stristr($cg_rd_page, 'http'))) {
            $cg_rd_page = 'https://t.me/s/' . $cg_rd_page;
        }

        //verify if page contains t.me/s and if not, ask the user to add a correct page URL
        if (!(stristr($cg_rd_page, 't.me/s/')  )) {
            echo '<br>Telegram page URL is not correct please visit telegram.com and get a correct one on the format https://t.me/s/id ';
            return false;
        }
        ...
        if (in_array('OPT_IT_CACHE', $camp_opt)) {
            ...
            if (wp_automatic_trim($after_tag) != '') {
                ...
                if ($after_md == $cg_rd_page_md) {
                    ...
                    $cg_rd_page .= 'before=' . $after_tag;
                }
            }
        }

        echo '<br>Telegram items url:' . $cg_rd_page;
        ...
        //extract elements with css class tgme_widget_message_wrap
        $dom = new DOMDocument();
        @$dom->loadHTML($exec);
        $xpath = new DOMXPath($dom);
        $elements = $xpath->query("//*[contains(concat(' ', normalize-space(@class), ' '), 'tgme_widget_message_wrap')]");
        $allItms = array();
        ...
        if (count($allItms) > 0) {
            ...
            //get first item
            $first_item = $allItms[0];

            //find numeric ID using REGEX data-post="on2club/228" should return 228
            preg_match_all('/data-post=".*?\/(\d+)"/', $first_item, $matches);

            //set after to the numeric ID
            $after = $matches[1][0];
        ...
        foreach ($allItms as $itemTxt) {
            ...
            $item['item_type'] = 'text'; //ini item type

            //extract the href of the link that has class named tgme_widget_message_date from itemTxt
            //dom
            $dom = new DOMDocument();
            ...
            $xpath = new DOMXPath($dom);
            $elements = $xpath->query("//*[contains(concat(' ', normalize-space(@class), ' '), 'tgme_widget_message_date')]");
            $item_link = $href = $elements->item(0)->getAttribute('href');

            //if href == 'https://t.me/on2club/227 continue
            //if($href != 'https://t.me/on2club/247') continue;

            //href is on the form https://t.me/on2club/227

            //set item_url and item_link to href

            $item['item_url'] = $href;
            $item['item_link'] = $href;

            //md5 the item url and set it as the item_link
            $id = md5($item['item_url']);
            $item['item_id'] = $id;
            ...
            $item['item_views'] = isset($elements->item(0)->nodeValue) ? $elements->item(0)->nodeValue : '';

            //extract video src URL using REGEX <video src="https://cdn4.telegram-cdn.org/file/c796994d28.mp4?token=LjWzVqucLqYGLdeMZkF5R_mQFyZ_AzOPBeXWr_l8SHoLGGSLfutDvkXUbAGID87HZfxdGPebm77iAy4XDFKMpQwmEhXkBZ8Edm5H_8uX7fTUrexECNpnocEVexxFtM3GMZElm9ATXaYhkJ-S4L7BJ_VufDXbYfMpDH7um_UZtgN4mVFocSewqcZSoGn2oMN4nsWU7eWMdXIiAOz2B-HAxqp0029h73iyPaYQvtMVyGIXSon88op1a8VIhikPocCch6HY9L1m8Iafz_IgWbCEyOAqaTBosQhiNbLCeNc2vhNlOFJEas1FGj_-YUgtRaxJ69eCg7JZ43j7ya5HdLxkyw" class="tgme_widget_message_video js-message_video" width="100%" height="100%"></video>
            ...
            if (stristr($itemTxt, 'link_preview_title')) {
                ...
                $item['item_link_title'] = isset($matches[1][0] ) ? $matches[1][0] : '';
                ...
                //if shared video, grab video thumb      <i class="link_preview_video_thumb" style="background-image:url('https://cdn4.telegram-cdn.org/file/Z
    ...
    public function telegram_get_post($camp)
    {
        ...
        foreach ($keywords as $keyword) {
            ...
            if (wp_automatic_trim($keyword) != '') {
                ...
                echo '<br>Posting from Telegram page:' . $cg_te_page;
                ...
                for ($i = 0; $i < $res_count; $i++) {
                    ...
                    if ($this->is_duplicate($t_link_url)) {
                        ...
                        echo '<br>Telegram item (' . $t_data['item_title'] . ') found cached but duplicated <a href="' . get_permalink($this->duplicate_id) . '">#' . $this->duplicate_id . '</a>';
                ...
                if (count($res) > 0) {
                    ...
                    if (wp_automatic_trim($temp['item_type']) == 'video' || isset($temp['item_embed_url'])) {
                        ...
                        $temp['item_embed'] = '<script async src="https://telegram.org/js/telegram-widget.js?22" data-telegram-post="' . $short_link . '" data-width="100%"></script>';
