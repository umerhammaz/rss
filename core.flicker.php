...
function flicker_fetch_items($keyword, $camp) {
	...
	}elseif(in_array('OPT_FL_GROUP', $camp_opt)){
		  echo '<br>Fetching images for specific group  ' . $camp_general ['cg_fl_group'];
		$flink = $flink . '&group_id=' . trim($camp_general ['cg_fl_group']);
		// if keyword *
		if (wp_automatic_trim( $keyword ) == '*') {
			  echo '<br>No filtering get all ';
		} else {
